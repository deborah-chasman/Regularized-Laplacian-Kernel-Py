...
for lam in 0.1 0.5 1 5 10 50 100; do
	if [ -a "tiny_${EX}_kernel_${lam}.npy" ]; then
		python ../get_kernel_scores.py tiny_${EX}_scores.tab tiny_${EX}.sif $lam tiny_${EX}_kernel_${lam}.npy --crossval > ${OUT}/${EX}_${lam}_example_output.tab	
	else
		python ../get_kernel_scores.py tiny_${EX}_scores.tab tiny_${EX}.sif $lam --crossval > ${OUT}/${EX}_${lam}_example_output.tab
	fi
done

# print only full scores from each lamda and header to terminal
cut -f 1,2,3 ${OUT}/${EX}_0.1_example_output.tab | grep -v "#" > ${TEMP}	
for lam in 0.5 1 5 10 50 100; do
